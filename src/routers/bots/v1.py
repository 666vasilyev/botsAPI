...
async def get_bots(session: Annotated[AsyncSession, Depends(get_session)]):
    return await get_all_bots(session)


@router.get("/{bot_id}", response_model=BotRead)
async def get_bot(bot_id: int, session: Annotated[AsyncSession, Depends(get_session)]):
...
async def delete_bots(
    bot_id: int, session: Annotated[AsyncSession, Depends(get_session)]
):
